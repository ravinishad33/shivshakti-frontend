...
export default function LabourProfile() {
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    const passwordPromise = axios.put(`http://localhost:5000/api/auth/profile/password`, {
  ...
  const handleRequestOtp = async () => {
    ...
    const otpPromise = axios.post(`http://localhost:5000/api/auth/profile/request-otp`, {
  ...
  const handleOtpVerifyAndSubmit = async (e) => {
    ...
    const verifyPromise = axios.put(`http://localhost:5000/api/auth/profile/verify-otp-reset`, {
      otp: otpData.otpCode,
  ...
  if (pageLoading) {
    return (
      <div className="min-h-[50vh] flex flex-col items-center justify-center text-slate-500 font-medium text-xs tracking-wider gap-3">
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto p-2 md:p-4 select-none font-sans text-slate-100">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 shadow-xl rounded-3xl p-6 flex flex-col items-center justify-between text-center gap-6 relative overflow-hidden">
          ...
          <div className="space-y-4 flex flex-col items-center w-full">
            {profile.profilePhoto ? (
              <img 
                ...
                alt="Labour Avatar" 
                className="w-24 h-24 rounded-2xl object-cover border border-slate-700 shadow-md bg-slate-950"
              />
            ) : (
              <div className="w-20 h-20 rounded-2xl bg-slate-950 border border-slate-800 text-white flex items-center justify-center text-2xl shadow-inner">
                👷
              </div>
            )}
            ...
            <div className="space-y-1">
              ...
              <div className="flex gap-1.5 justify-center items-center">
                <span className="inline-block text-[9px] bg-orange-500/10 text-orange-400 font-black border border-orange-500/20 px-2.5 py-0.5 rounded-md uppercase tracking-widest">
                  {profile.role === 'labour' ? 'Workforce' : 'Admin'}
                </span>
          ...
          <div className="w-full pt-4 border-t border-slate-800/80 text-left space-y-3 text-xs text-slate-400">
            <div className="flex justify-between items-center">
              <span className="text-slate-500 font-semibold">System Access ID:</span>
              <span className="font-mono font-bold text-white bg-slate-950 px-2 py-0.5 rounded border border-slate-800">{profile.identityId}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500 font-semibold">Mobile Contact:</span>
              <span className="font-bold text-slate-200">{profile.mobile}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500 font-semibold">Daily Wage Rate:</span>
              <span className="font-bold text-orange-400">₹{profile.dailyWage || 0}<span className="text-[10px] text-slate-500 font-mono font-medium">/Day</span></span>
            </div>
            <div className="flex justify-between">
            ...
            {profile.aadhaarPhoto && (
              <div className="pt-2">
                <a 
                  href={`http://localhost:5000/${profile.aadhaarPhoto}`} 
                  target="_blank" 
                  rel="noreferrer"
                  className="w-full inline-block text-center bg-slate-950 hover:bg-slate-800/60 border border-slate-800 text-slate-300 font-bold py-2.5 rounded-xl text-[10px] uppercase tracking-wider transition-colors"
                >
                  📄 show Adhar Card
                </a>
              </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 shadow-xl rounded-3xl p-5 md:p-6 space-y-4">
            <h5 className="text-xs font-black text-slate-400 uppercase tracking-wider border-b border-slate-800 pb-2 flex items-center gap-1.5">
              <span>🔒</span> Passkey Configuration Hub
            </h5>
            ...
            <div className="flex border-b border-slate-800 text-[11px] md:text-xs font-bold tracking-wide">
              <button 
                ...
                className={`py-2 px-4 border-b-2 transition-all uppercase tracking-wider ${
                  activeTab === 'password' 
              ...
              <form onSubmit={handlePasswordSubmit} className="space-y-4 pt-2">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      placeholder="••••••••"
                      className="w-full bg-slate-950/60 border border-slate-800 text-white focus:border-orange-500 rounded-xl px-3 py-2.5 text-xs outline-none transition-all placeholder:text-slate-700"
                      required
                    />
                  </div>
                  <div className="space-y-1">
                ...
                <div className="flex justify-end pt-2">
                  <button
                    ...
                    className="w-full sm:w-auto bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-bold px-5 py-2.5 rounded-xl text-[11px] uppercase tracking-wider shadow-lg hover:shadow-orange-500/5 active:scale-[0.98] transition-all disabled:opacity-50"
                  >
                ...
                {!otpSent ? (
                  <div className="bg-slate-950/40 p-4 rounded-2xl border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="space-y-1 flex-1">
                      <p className="text-xs font-bold text-slate-200">Dispatch Verification Challenge OTP</p>
                      <input 
                        type="email"
                        value={otpData.emailInput}
                        onChange={(e) => setOtpData({ ...otpData, emailInput: e.target.value })}
                        placeholder="Enter your email box address..."
                        className="w-full bg-slate-950 border border-slate-800 text-white focus:border-orange-500 rounded-xl px-3 py-2 text-xs outline-none mt-1 transition-all placeholder:text-slate-700"
                      />
                    </div>
                    <button
                      type="button"
                      ...
                      disabled={actionLoading}
                      className="bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] uppercase tracking-wider font-black px-4 py-2.5 rounded-xl shadow-md transition-all self-end sm:self-center active:scale-[0.98] disabled:opacity-50"
                    >
                      Send OTP Code
                    </button>
                  </div>
                  ...
                  <form onSubmit={handleOtpVerifyAndSubmit} className="space-y-4 border border-dashed border-orange-500/20 p-4 rounded-2xl bg-orange-500/[0.02]">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div className="space-y-1">
                        <label className="block text-[10px] font-bold text-orange-400 uppercase tracking-wider pl-0.5">Enter 6-Digit OTP</label>
                        <input
                          type="text"
                          maxLength="6"
                    ...
                    <div className="flex justify-between items-center pt-2 gap-4 text-xs font-semibold">
                      <button 
                        type="button" 
                        onClick={() => setOtpSent(false)} 
                        className="text-[11px] text-orange-400 hover:text-orange-300 font-bold underline"
                      >
                      ...
                      <button
                        type="submit"
                        disabled={actionLoading}
                        className="bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-bold px-5 py-2.5 rounded-xl text-[11px] uppercase tracking-wider shadow-lg active:scale-[0.98] transition-all disabled:opacity-50"
                      >
