...
export default function AdminLayout() {
  ...
  const menuItems = [
    ...
    { path: '/admin/labour', label: 'Manage Labour', icon: <HardHat className="w-4 h-4" /> },
    { path: '/admin/attendance', label: 'Attendance', icon: <CalendarCheck2 className="w-4 h-4" /> },
    { path: '/admin/cashbook', label: 'Cashbook', icon: <BookOpen className="w-4 h-4" /> },
    { path: '/admin/salary', label: 'Manage Salary', icon: <Banknote className="w-4 h-4" /> },
    { path: '/admin/sites', label: 'Manage Sites', icon: <Building2 className="w-4 h-4" /> },
      ...
      `}>
        ...
        <div className="p-6 border-b border-slate-800 bg-slate-950/40 flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] font-bold tracking-widest text-slate-400 uppercase mt-0.5">Construction</p>
          </div>
        ...
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-800">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <div className={`${isActive ? 'text-white' : 'text-slate-500 group-hover:text-white'} transition-colors shrink-0`}>
                  {item.icon}
                </div>
        ...
        <header className="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-4 sm:px-8 shadow-sm z-10 w-full">
          ...
          <div className="flex items-center gap-3 sm:gap-4 shrink-0">
            <div className="text-right hidden sm:block">
              <p className="text-xs font-black text-slate-900 leading-none">{user?.name || 'Administrator'}</p>
